...
C_out = (
    (k_mid - k_out) * r_p**2 / 2.0
    + (b_mid - b_out) * np.log(r_p)
    + C_mid
)

# Ten nonzero relaxation modes:
# m = 2,3,...,11
NUMBER_OF_MODES = 10

# Radius used for the right-side shooting.
# R_SHOOT=15 is sufficiently large for the present parameter set.
R_SHOOT = 15.0
EPS = 1.0e-6

# The second initial inverse temperature used in Fig. 7.
# beta_ini_star is determined automatically as the peak position of a2.
...
# ============================================================
# Effective Schrodinger potential V_S(r)
# ============================================================
def VS_scalar(r):
    if r < r_m:
        k, b = k_in, b_in
...
#
# phi'' + phi'/r + (lambda - V_S)/T phi = 0
#
# Left solution:
#   regular at r=0.
#
# Right solution:
#   decaying asymptotic solution at large r.
#
# Eigenvalues are obtained from the vanishing Wronskian at r=r_p.
# ============================================================
...
def ode_rhs(r, y, lam):
    ...
    return [
        dphi,
        -dphi / r - (lam - VS_scalar(r)) / T * phi,
    ]


def integrate_left(lam, dense_output=False):
    # Regular expansion near the origin:
    # phi = 1 + c r^2 + O(r^4),
    # c = -(lambda + k_in)/(4T).
    c = -(lam + k_in) / (4.0 * T)
...
def integrate_right(lam, dense_output=False):
    # For large r,
    # phi ~ r^(nu-2mu) exp(-gamma r^2/2).
    power = nu_out - 2.0 * mu_out(lam)

    # Overall normalization is arbitrary in shooting.
    phi_R = 1.0
    dphi_R = (
        power / R_SHOOT - gamma_out * R_SHOOT
    ) * phi_R
...
# For the present parameter set, the ten relaxation roots are
# well separated.  Using broad brackets makes the shooting calculation
# much faster and still determines each root by brentq.
ROOT_BRACKETS = [
    (0.80, 1.10),
    (1.80, 2.20),
    (3.20, 3.70),
...
def find_eigenvalues(number_of_modes):
    ...
    for left, right in ROOT_BRACKETS[:number_of_modes]:
        ...
        if f_left * f_right >= 0.0:
            raise RuntimeError(
                f"No sign change in bracket ({left}, {right})."
            )
...
for j, lam in enumerate(eigenvalues):
    print(
        f"m={j+2:2d}  "
        f"lambda_{j+2} = {lam:.12f}"
    )

print("\nThe previously missed eigenvalue is")
print(
    f"lambda_3 = {eigenvalues[1]:.12f}"
)

# ============================================================
# Construct normalized eigenfunctions numerically
# ============================================================
...
def eigenfunction_on_grid(lam):
    ...
    # Determine the scale factor for the right solution.
    # Use whichever component is safer numerically.
    ...
    phi[mask_left] = left_sol.sol(
        r_grid[mask_left]
    )[0]

    phi[mask_right] = (
        scale_right
        * right_sol.sol(r_grid[mask_right])[0]
    )

    # Normalize:
    # 2 pi integral_0^infty dr r phi^2 = 1
    norm = 2.0 * np.pi * np.trapezoid(
        r_grid * phi**2,
        r_grid,
    )
    ...
    # Fix an arbitrary sign convention.
    if phi[0] < 0.0:
        phi *= -1.0
...
# ============================================================
# Partition function and mode amplitudes
# ============================================================
V_grid = V(r_grid)
...
def Z(beta_value):
    return 2.0 * np.pi * np.trapezoid(
        r_grid * np.exp(-beta_value * V_grid),
        r_grid,
    )
...
def raw_projection(j, beta_ini):
    ...
    return (
        2.0
        * np.pi
        * np.trapezoid(integrand, r_grid)
        / Z(beta_ini)
    )


def mode_amplitude(j, beta_ini):
    # Subtract the small numerical projection at beta_ini=beta.
    return (
        raw_projection(j, beta_ini)
        - raw_projection(j, beta)
    )


# ============================================================
# Locate beta_ini^* from the maximum of a2(beta_ini)
# ============================================================
...
print("======================================================")
print(
    f"beta_ini_star = {beta_ini_star:.12f}"
)
print(
    f"a2_peak       = {a2_of_beta(beta_ini_star):+.12e}"
)
print(
    f"beta_ini_sharp = {beta_ini_sharp:.12f}"
)


# ============================================================
# Mode amplitudes for the two initial states
# ============================================================
a_star = np.array(
    [
        mode_amplitude(j, beta_ini_star)
...
a_sharp = np.array(
    [
        mode_amplitude(j, beta_ini_sharp)
        for j in range(NUMBER_OF_MODES)
    ]
)
...
for j in range(NUMBER_OF_MODES):
    print(
        f"m={j+2:2d}, "
        f"lambda={eigenvalues[j]:.9f}, "
        f"a_sharp={a_sharp[j]:+.8e}, "
        f"a_star={a_star[j]:+.8e}"
    )
...
#
# Compare:
#   (1) two-mode quadratic      : m=2,3;      O(delta P^2)
#   (2) four-mode quadratic     : m=2,...,5;  O(delta P^2)
#   (3) ten-mode quadratic      : m=2,...,11; O(delta P^2)
#   (4) two-mode nonlinear      : m=2,3;      O(delta P^4)
#   (5) ten-mode direct KL      : optional numerical check
# ============================================================
...
# Appendix-E nonlinear overlap coefficients
phi2 = phi_modes[0]
phi3 = phi_modes[1]
lam2 = eigenvalues[0]
lam3 = eigenvalues[1]
...
def E_overlap(phi_k, phi_l, phi_m, phi_n):
    integrand = (
        r_grid * np.exp(beta * V_grid)
        * phi_k * phi_l * phi_m * phi_n
    )
...
def DKL_nonlinear_two_mode(t, amplitudes):
    ...
    D2 = 0.5 * Cstar * (
        a2**2 * np.exp(-2.0 * lam2 * t)
        + a3**2 * np.exp(-2.0 * lam3 * t)
    )

    D3 = -(1.0/6.0) * (
        D222 * a2**3 * np.exp(-3.0 * lam2 * t)
        + 3.0 * D223 * a2**2 * a3 * np.exp(-(2.0*lam2 + lam3)*t)
        + 3.0 * D233 * a2 * a3**2 * np.exp(-(lam2 + 2.0*lam3)*t)
        + D333 * a3**3 * np.exp(-3.0 * lam3 * t)
    )

    D4 = (1.0/12.0) * (
        E2222 * a2**4 * np.exp(-4.0 * lam2 * t)
        + 4.0 * E2223 * a2**3 * a3 * np.exp(-(3.0*lam2 + lam3)*t)
        + 6.0 * E2233 * a2**2 * a3**2 * np.exp(-2.0*(lam2 + lam3)*t)
        + 4.0 * E2333 * a2 * a3**3 * np.exp(-(lam2 + 3.0*lam3)*t)
        + E3333 * a3**4 * np.exp(-4.0 * lam3 * t)
    )
...
Delta_nonlinear = (
    DKL_nonlinear_two_mode(t, a_sharp)
    - DKL_nonlinear_two_mode(t, a_star)
) / Cstar


# Optional direct KL check from ten-mode reconstructed P(r,t)
...
Delta_direct_ten = np.array([
    (DKL_direct(a_sharp, tt, 10) - DKL_direct(a_star, tt, 10))/Cstar
    for tt in t
])
...
fig.tight_layout()
fig.savefig('Fig7_compare_2_4_10_nonlinear.eps', bbox_inches='tight')
fig.savefig('Fig7_compare_2_4_10_nonlinear.png', dpi=300, bbox_inches='tight')
...
#ax.plot(t, Delta_four, linewidth=2.2, linestyle='--')
ax.plot(t, Delta_ten, linewidth=2.0, linestyle='-.')
ax.plot(t, Delta_nonlinear, linewidth=2.2, linestyle=':')
#ax.plot(t, Delta_direct_ten, linewidth=1.8, linestyle=(0,(5,2,1,2)))
ax.set_xlabel(r'$t$', fontsize=24)
ax.set_ylabel(r'$\Delta D_{\rm KL}/C^*$', fontsize=24)
ax.set_xlim(2.5, 3.5)
...
#ax.legend(fontsize=13, frameon=True)
fig.tight_layout()
fig.savefig('Fig7_compare_2_4_10_nonlinear_zoom.eps', bbox_inches='tight')
fig.savefig('Fig7_compare_2_4_10_nonlinear_zoom.png', dpi=300, bbox_inches='tight')
plt.show()
