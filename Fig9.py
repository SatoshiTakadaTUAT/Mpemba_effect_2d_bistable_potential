...
zero = []
for lam in lam_list:
    detM_list.append(detM(lam))
    detM_ = detM(lam)
    if detM_ < 0:
        log_detM_list.append(-np.log(detM_))
    else:
        log_detM_list.append(1e-10)
    zero.append(0)
...
plt.grid()
plt.tight_layout()
plt.savefig(f'Fig9a.eps', bbox_inches='tight')
...
plt.grid()
plt.tight_layout()
plt.savefig(f'Fig9b.eps', bbox_inches='tight')
